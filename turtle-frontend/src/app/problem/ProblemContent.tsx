...
export default function ProblemContent({
  ...
  const updateStoryQueue = (newStory: Story) => {
    const cachedStories = JSON.parse(
      localStorage.getItem("storyQueue") || "[]"
    );
      ...
      if (storyId) {
        ...
        if (cachedStory) {
          ...
          const storyData = JSON.parse(
            localStorage.getItem(`story_${cachedStory.id}`) || "{}"
          );
          if (storyData.qnas) {
          ...
          if (!storyData.startTime) {
            ...
            localStorage.setItem(`story_${cachedStory.id}`, JSON.stringify(storyData));
          }
        } else {
          try {
            const fetchedStory = await fetchStoryById(Number(storyId));
            const storyData = JSON.parse(
              localStorage.getItem(`story_${fetchedStory.id}`) || "{}"
            );
            if (storyData.qnas) {
              setQnas(storyData.qnas);
            }
            if (!storyData.startTime) {
              ...
              const currentDate = toKSTISOStringFull(now);
              storyData.startTime = currentDate;
              localStorage.setItem(`story_${fetchedStory.id}`, JSON.stringify(storyData));
            }
        ...
        if (cachedTodayStory) {
          ...
          const storyData = JSON.parse(
            localStorage.getItem(`story_${cachedTodayStory.id}`) || "{}"
          );
          if (storyData.qnas) {
            setQnas(storyData.qnas);
          }
          if (!storyData.startTime) {
            ...
            const currentDate = toKSTISOStringFull(now);
            storyData.startTime = currentDate;
            localStorage.setItem(`story_${cachedTodayStory.id}`, JSON.stringify(storyData));
          }
        } else {
          try {
            ...
            const storyData = JSON.parse(
              localStorage.getItem(`story_${todayStory.id}`) || "{}"
            );
            if (storyData.qnas) {
              setQnas(storyData.qnas);
            }
            if (!storyData.startTime) {
              ...
              const currentDate = toKSTISOStringFull(now);
              storyData.startTime = currentDate;
              localStorage.setItem(`story_${todayStory.id}`, JSON.stringify(storyData));
            }
            setStory(todayStory);
  ...
  const saveQnasToLocalStorage = (qnas: Qna[]) => {
    const storyData = JSON.parse(localStorage.getItem(`story_${story?.id || "today"}`) || "{}");
    storyData.qnas = qnas;
